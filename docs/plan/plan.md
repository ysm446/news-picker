...
## 実装済み: カードのサムネイル画像(2026-07-06 追加、同日完了)

以下の設計から次の点を変更して実装済み:
- 表示位置はカード右の小サムネイルではなく**タイトル上のフル幅画像**(1.91:1、ユーザー指定)
- og:image 補完は未実装(fetch_page が生 HTML を返さないため見送り。ddgs image +
  RSS media で大半カバーできる。必要になったら fetch_page 改修とセットで)
- 環境設定に「カードにサムネイル画像を表示する」トグルを追加済み(既定オン)
検証: 実画像でダウンロード・縮小・キャッシュヒット・削除を確認。

## (元の計画) カードのサムネイル画像

ニュース元の画像をカードに小さく表示する。画像はローカルに一時キャッシュし、
記事のパージと一緒に削除する(「一時的に取得」の要件)。

### 画像の入手経路(取得コスト順)

1. **ddgs ニュース検索の `image` フィールド**: 検索結果に含まれるが現在は捨てている。
   拾うだけなので追加コストほぼゼロ(二層カデンスの安い側のまま)。
2. **RSS の media:thumbnail / enclosure**: feedparser が公開している。同上。
3. **og:image**(記事ページのメタタグ): EnrichWorker が trafilatura で本文取得する際に
   ついでに抽出(クリック後の補完。1/2 で取れなかった記事用)。

### 設計方針

- **DB**: `articles.image_url` カラム追加。vault MD の frontmatter にも記録
  (rebuild-index で復元可能に = 「MD が真実の源」の原則)。
- **キャッシュ**: ホットリンクはせず、バックエンドの `GET /articles/{id}/thumb` が
  初回アクセス時にダウンロード → `data/cache/images/` に縮小保存(長辺 ~200px、
  Pillow 使用・要 requirements 追加)して返す。以降はキャッシュ配信。
  - 利点: 表示のたびに外部ホストへ飛ばない、リンク切れに強い、プライバシー面も良い
- **削除**: CleanupWorker のパージ・非表示処理でキャッシュ画像も削除。
- **UI**: カード右側に小さなサムネイル(~56px 角、object-fit: cover)。画像なし
  カードとの高さ差が出ないようレイアウト調整。詳細パネルでは大きめに表示してもよい。
  環境設定に「サムネイル表示」トグルを設ける(密度重視のユーザーはオフにできる)。

### 留意点

- 画像 URL の品質はソース依存(ロゴだけ・トラッキングピクセル等のノイズがある)。
  極端に小さい画像は捨てるなどの下限フィルタを入れる。
- カード密度が下がるため、まず1カテゴリで試して見た目を確認してから全体に広げる。

## 実装予定: 保存時に自動 enrich して MD 化(2026-07-07 追加)

保存記事の永続性を確実にする。現状、記事 MD は詳細生成(カードを開いた)時に
しか書き出されないため、「保存したが一度も開いていない」記事は SQLite にしか
存在せず、`POST /admin/rebuild-index`(MD からの全再構築)で復元できない。

- **変更**: `POST /articles/{id}/save` の保存時(トグルオンの側)に、未 enrich
  なら EnrichWorker のキューに積む。enrich 完了で MD が vault に書かれ、
  「保存 = vault に確実に書き残す」というセマンティクスになる。
- 数行の変更で済む見込み(save エンドポイントから `enricher.enqueue()` を呼ぶだけ)。
- 留意: 保存操作に LLM 生成が1回付随する(バックグラウンドなので UI は待たない)。

## 保留: モデルの一本化 + 上部モデル選択バー(2026-07-12 検討、保留)

ニュース処理(採点・要約)と深堀りチャットのモデルを1つに統一し、2ポート
(standard 8081 / deep 8082)構成をやめる案。lm-chat の ModelBar /
ModelPickerModal 風の選択 UI を上部中央に置き、models/ 配下の GGUF を
ポップアップで即切り替えできるようにする。

- 設計案: 起動時は設定画面の「既定のモデル」をロード。バーでの切り替えは
  一時的(llama-server を再起動、既定は変えない)。アンロードボタンで VRAM 解放。
  ctx は 32k に統一。`model_deep`・deep ロール・ステータスバーの deep トグルは廃止。
- 根拠: 深堀りオフ時は常駐モデルがチャット代行する実績パスが既にあり、
  一本化はそこに寄せるだけ。手動切り替えで「普段 9B / 深堀り日は 35B」運用が可能。
- 留意点: 35B 選択中は背景処理(採点・翻訳・要約)も 35B が担い重くなる。
  切り替え中(数十秒)は LLM 加工が一時停止(タイトル取り込みは動き続ける)。

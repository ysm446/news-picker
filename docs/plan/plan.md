...
## 実装済み: 保存時に自動 enrich して MD 化(2026-07-07 追加、2026-07-12 完了)

保存記事の永続性を確実にする。記事 MD は詳細生成時にしか書き出されないため、
「保存したが一度も開いていない」記事は rebuild で復元できなかった。

- 実装: save エンドポイントが未 enrich なら `enricher.enqueue()` を呼ぶ。
- あわせて 2026-07-12 のレビュー対応で「状態変更の MD 書き戻し」「hide 時の
  MD 削除」「rebuild の tombstone 照合・トランザクション化・埋め込み復元」も
  実装済み(詳細は changelog 参照)。「MD が真実の源」が rebuild を跨いで成立する。

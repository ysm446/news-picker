...
- **コードレビュー対応(2026-07-12)**: 3方向(バックエンド / フロント / Electron)の
  レビューで見つかった問題の修正一式。最重要は「MD が真実の源」の整合性回復
  (状態変更の MD 書き戻し、hide 時の MD 削除、rebuild の tombstone 照合・
  1トランザクション化・埋め込み復元、保存時の自動 enrich)。ほか openExternal の
  プロトコル制限、ChatPanel の busy スタック/中断/履歴混入、終了時の llama-server
  孤児化対策、SSE/enrich キューのスレッド境界修正。詳細は changelog 参照
...
- レビュー指摘の残件(中程度以下): SSE 再接続時の loadAll 巻き戻し、API の
  無認証 + CORS `Origin: null` 許可、stop.bat の無差別キル、カード一覧の
  メモ化(React.memo)、curated イベントの詳細パネル未反映など。
